...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < T; i++) {
			...
			double x1 = ((P * 1.0) / 12.0 - Math.sqrt((1.0 * P / 12.0) * (1.0 * P / 12.0) - (1.0 * S) / 6.0));
			double x2 = ((P * 1.0) / 12.0 + Math.sqrt((1.0 * P / 12.0) * (1.0 * P / 12.0) - (1.0 * S) / 6.0));
			double x = x1;
			...
			;
			double y = ((1.0 * P / 4.0) - (2.0 * x));
			// System.out.println(x1 + " " + x2 + " " +x + " " + y);
			double sol = x * x * y;

			sol = (int) Math.round((sol * 100.00) + 0.005);
			System.out.println(sol / 100.0);
		}
	}
